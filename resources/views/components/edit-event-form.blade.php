...
<form method="POST" action="/event/update/{{ $event->id }}" class="w-full max-w-md bg-white p-6 rounded-lg shadow">
    @csrf
    @method('put')

    @include('partials.session-message')

    <x-section-header title="Edit event" />
    ...
    <div class="mb-4">
        ...
        <input type="datetime-local" name="event_date" id="event_date" required value="{{ old('event_date', $event->event_date) }}"
    ...
    <div class="mb-4">
        ...
        <textarea name="details" id="details" required
            class="mt-1 block w-full px-4 py-2 border @error('details') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('details', $event->details) }}</textarea>
    ...
    <div class="mb-4">
        ...
        <input type="number" name="ticket_price" id="ticket_price" required value="{{ old('ticket_price', $event->ticket_price) }}"
    ...
    <div class="mb-4">
        ...
        <input type="date" name="booking_start_date" id="booking_start_date" required
            value="{{ old('booking_start_date', $event->booking_start_date) }}"
    ...
    <div class="mb-4">
        ...
        <input type="date" name="booking_deadline_date" id="booking_deadline_date" required
            value="{{ old('booking_deadline_date', $event->booking_deadline_date) }}"
